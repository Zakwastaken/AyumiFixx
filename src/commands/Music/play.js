...
module.exports = {
   ...
   execute: async (message, args, client, prefix) => {
    ...
    if (!message.guild.me.permissionsIn(channel).has([Permissions.FLAGS.CONNECT, Permissions.FLAGS.SPEAK])) return message.channel.send({embeds: [new MessageEmbed().setColor(client.embedColor).setDescription(`I don't have enough permissions connect your voice channel please give me permission \`CONNECT\` or \`SPEAK\`.`)]});
   
    let SearchString = args.join(" ");
    const emojiaddsong = message.client.emoji.addsong;
    const emojiplaylist = message.client.emoji.playlist;
    
    if(SearchString.startsWith("https://open.spotify.com/playlist/")) message.reply({ embeds: [new MessageEmbed().setAuthor(`Spotify`, "https://i.imgur.com/cK7XIkw.png").setColor("GREEN").setTimestamp().setDescription(`\`\`\`Started playing music from Spotify.\`\`\``)]}).then(msg => { setTimeout(() => {msg.delete()}, 10000);
       })
    ...
    try {
      ...
      if (Searched.loadType === "PLAYLIST_LOADED") {
       ...
       if (!player.playing && !player.paused && !player.queue.size)
         ...
         return message.channel.send({ embeds: [new MessageEmbed().setColor("#34eb95").setTimestamp().setDescription('\`\`\`There were no results found!\`\`\`')]});
        }
      } else {
        let Searched = await player.search(SearchString, message.author);
         if (!player)
           return message.channel.send({ embeds: [new MessageEmbed().setColor("#34eb95").setTimestamp().setDescription("\`\`\`Nothing is playing right now ...\`\`\`")]});

         if (Searched.loadType === "NO_MATCHES")
           return message.channel.send({ embeds: [new MessageEmbed()].setColor("#34eb95").setTimestamp().setDescription(`**No matches found for - [this]${SearchString}!**`)});
